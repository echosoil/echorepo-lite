...
def _ts_to_iso(ts):
    ...
    Accepts:
      - google.protobuf.timestamp_pb2.Timestamp (has .seconds, .nanos)
    ...
    # datetime from Firestore?
    if hasattr(ts, "isoformat"):
        # ensure tz-aware so it's unambiguous
        dt = ts
...
def fetch_samples_flat() -> pd.DataFrame:
    ...
    for doc in samples_ref.stream():
        ...
        # base row — ADD Firestore metadata here
        row = {
            "sampleId": doc.id,
            "userId": (
            ...
            # NEW: actual Firestore timestamps
            "fs_createdAt": _ts_to_iso(getattr(doc, "create_time", None)),
            "fs_updatedAt": _ts_to_iso(getattr(doc, "update_time", None)),
            # you could also store read time if you want:
            # "fs_readAt": _ts_to_iso(getattr(doc, "read_time", None)),
        }
...
def main():
    ...
    # 3) Distinct users (you were just dumping all Auth emails)
