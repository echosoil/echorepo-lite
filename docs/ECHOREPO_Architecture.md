...
5. Automated publishing synchronization with external storage providers (e.g. Zenodo).
---
...
| **Gateway / Edge** | NGINX, rate‑limit, metrics | Docker | TLS termination, routing, token verification, circuit‑breaking. |
| **Core Micro‑services** | Ingestion, Kit‑Registry, Data‑Processor, Visualisation API | FastAPI/REST | Validate payloads, store data, business rules, expose rich queries. |
| **Data Stores** | PostgreSQL (metadata+lab. results), Object Storage (media) | Helm‑deployed DBs, S3‑compatible storage | Durable storage, retention policies, backups. |
| **Web Front‑end** | Flask, Dashboards, Map overlays | TypeScript, D3/Leaflet | Visualise datasets, export reports, admin console. |
| **Observability** |  Grafana | CNCF stack | Logs, metrics, traces, alerting. |
