...
def _qa_status_from_row(row) -> str | None:
    """
    Build qa_status for legacy SQLite GeoJSON.
    Uses existing qa_status if present, otherwise derives it from
    wrong_coordinates + coordinate_check_reason.
    """
    keys = row.keys() if hasattr(row, "keys") else []

    qa = ""
    if "qa_status" in keys:
        qa = str(row["qa_status"] or "").strip()

    if qa:
        return qa

    wrong = False
    if "wrong_coordinates" in keys:
        wrong = _truthy_flag(row["wrong_coordinates"])

    if wrong:
        reason = ""
        if "coordinate_check_reason" in keys:
            reason = str(row["coordinate_check_reason"] or "").strip()
