...
ALIASES = {
    ...
    "russia":"Russian Federation","czech republic":"Czechia","ivory coast":"Côte d’Ivoire",
    ...
    "moldova":"Moldova, Republic of","bolivia":"Bolivia, Plurinational State of",
    "tanzania":"Tanzania, United Republic of","palestine":"Palestine, State of",
    "laos":"Lao People’s Democratic Republic","vietnam":"Viet Nam",
...
def country_to_iso2(name: str) -> str | None:
    ...
    s = str(name).strip()
    s_l = s.lower()
    if s_l in ALIASES: s = ALIASES[s_l]
    try:
        c = pycountry.countries.lookup(s); return c.alpha_2
    except LookupError:
        s2 = re.sub(r"[’`]", "'", s).strip()
        try: return pycountry.countries.lookup(s2).alpha_2
