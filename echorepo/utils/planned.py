...
def _split_planned(s: str) -> set[str]:
    if not s or not str(s).strip(): return set()
    parts = re.split(r"[;,]", str(s))
    out = set(); 
    for p in parts:
        iso2 = country_to_iso2(p.strip())
        if iso2: out.add(iso2)
    return out
...
def load_qr_to_planned() -> dict[str, set[str]]:
    ...
    col_qr = None; col_countries = None
    for c in df.columns:
        cl = c.strip().lower()
        if cl in ("qr code","qr_code","qr"): col_qr = c
        if cl in ("country planned","planned country","countries planned"): col_countries = c
    if not col_qr or not col_countries: return {}
